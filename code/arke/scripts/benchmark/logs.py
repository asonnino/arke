...
class LogParser:
    def __init__(self, clients, shards, num_nodes=0, faults=0):
        ...
        self.rate, self.start, misses, sent_samples, certificates, acks = \
            zip(*results)
        ...
        self.acks = {k: v for x in acks for k, v in x.items()}
        ...
        shards_ips, _commits = zip(*results)
    ...
    def _keep_earliest_validity(self, input):
        ...
        if isinstance(self.committee_size, int):
            threshold = int((self.committee_size + 2) / 3)
        else:
            threshold = 1
    ...
    def _keep_earliest_quorum(self, input):
        ...
        if isinstance(self.committee_size, int):
            threshold = int((2 * self.committee_size) / 3)
        else:
            threshold = 1
    ...
    def _parse_clients(self, log):
        ...
        samples = {d: self._to_posix(t) for t, d in tmp}

        tmp = findall(r'(.*Z) .* Assembled certificate (.*)', log)
        tmp = [(d, self._to_posix(t)) for t, d in tmp]
        certificates = self._keep_earliest([tmp])  # Unnecessary

        tmp = findall(
            r'(.*Z) .* Acknowledged certificate (.*)', log)
        tmp = [(d, self._to_posix(t)) for t, d in tmp]
        acks = self._keep_earliest([tmp])  # Unnecessary

        return rate, start, misses, samples, certificates, acks
    ...
    def _parse_shards(self, log):
        ...
        tmp = findall(r'(.*Z) .* certificate (.*)', log)
        tmp = [(d, self._to_posix(t)) for t, d in tmp]
        certificates = self._keep_earliest([tmp])  # Unnecessary
    ...
    def _end_to_end_throughput(self):
        ...
        start, end = min(self.start), max(self.acks.values())
        duration = end - start
        txs = len(self.acks)
        tps = txs / duration
        return tps, duration
