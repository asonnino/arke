...
class Bench:
    ...
    def install(self):
        if self.settings.deploy_key:
            hosts = self.manager.hosts(flat=True)
            progress = progress_bar(hosts, prefix='Uploading deployment key:')
            for ip in progress:
                c = Connection(ip, user='ubuntu', connect_kwargs=self.connect)
                c.run('mkdir -p ~/.ssh', hide=True)
                c.run(
                    'ssh-keyscan github.com >> .ssh/known_hosts', hide=True
                )
                c.put(self.settings.deploy_key, './.ssh/id_rsa')
    ...
    def _update(self, hosts, collocate):
        ...
        cmd = [
            ...
            'source $HOME/.cargo/env',
            f'(cd {self.settings.repo_name}/code/{self.settings.repo_name} && {CommandMaker.compile()})',
            CommandMaker.alias_binaries(
                f'./{self.settings.repo_name}/code/{self.settings.repo_name}/target/release/'
            )
        ]
