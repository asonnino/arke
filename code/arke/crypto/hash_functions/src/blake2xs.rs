//! File adapted from <https://github.com/celo-org/celo-bls-snark-rs/blob/master/crates/bls-crypto/src/hashers/direct.rs>

use crate::{FixedLengthHash, HashError};
use blake2s_simd::Params;
use byteorder::{LittleEndian, WriteBytesExt};

/// Extensible-output function (XOF) from the Blake2X hash family.
/// The Blake2Xs instance is a Blake2X hash derived from a Blake2s instance as described in <https://www.blake2.net/blake2x.pdf>.
#[derive(Clone)]
pub struct Blake2Xs;

pub fn xof_digest_length_to_node_offset(
    node_offset: u64,
...
impl FixedLengthHash for Blake2Xs {
...
impl Blake2Xs {
    ...
    pub fn xof(
        ...
        if domain.len() > 8 {
            return Err(HashError::DomainTooLarge(domain.len()));
        }
...
mod test {
    ...
    fn test_crh_empty() {
        let msg: Vec<u8> = vec![];
        let result = Blake2Xs::crh(&[], &msg, 96).unwrap();
        assert_eq!(
            hex::encode(result),
    ...
    fn test_xof_random_96() {
        ...
        let result = Blake2Xs::crh(&[], &msg, 96).unwrap();
        let xof_result = Blake2Xs::xof(b"ULforxof", &result, 96).unwrap();
        assert_eq!(
    ...
    fn test_hash_random() {
        ...
        let result = Blake2Xs::hash(b"ULforxof", &msg, 96).unwrap();
