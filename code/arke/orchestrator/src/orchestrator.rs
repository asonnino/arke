...
impl<P: ProtocolCommands<T> + ProtocolMetrics, T: BenchmarkType> Orchestrator<P, T> {
    ...
    async fn boot_nodes(
        ...
        let context = CommandContext::new()
            ...
            .with_execute_from_path(self.settings.path_to_root());
        self.ssh_manager
    ...
    pub async fn configure(&self, parameters: &BenchmarkParameters<T>) -> TestbedResult<()> {
        ...
        let execution_path = self.settings.path_to_root();
        let context = CommandContext::new().with_execute_from_path(execution_path);
