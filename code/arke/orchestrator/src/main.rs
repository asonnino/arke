...
use protocol::arke::{ArkeBenchmarkType, ArkeProtocol};
...
async fn run<C: ServerProviderClient>(settings: Settings, client: C, opts: Opts) -> Result<()> {
    ...
    match opts.operation {
        ...
        Operation::Benchmark {
            ...
            let arke_benchmark_type = match benchmark_type {
                Some(t) => ArkeBenchmarkType::from_str(&t)?,
                None => ArkeBenchmarkType::default(),
            };
            let protocol_commands = ArkeProtocol::new(&settings);
            ...
            let generator = BenchmarkParametersGenerator::new(committee, load)
                .with_benchmark_type(arke_benchmark_type)
        ...
        Operation::Plot { x_lim, y_lim } => Plotter::<ArkeBenchmarkType>::new(settings)
            .with_x_lim(x_lim)
            .with_y_lim(y_lim)
            .load_measurements()
        ...
        Operation::Summarize { path } => {
            MeasurementsCollection::<ArkeBenchmarkType>::load(path)?.display_summary()
        }
    }
    Ok(())
}
